...
def test_optional_submodel():
    house = pydantic_plus.obj_from_toml(HOUSE_TOML_PATH, HouseMaybeDim, convert_strpaths=True)
    assert house.dimensions == Dimensions(**HOUSE_DIMENSIONS)


def test_submodel_list():
    house = pydantic_plus.obj_from_toml(HOUSE_TOML_LISTDIM_PATH, HouseListDim, convert_strpaths=True)
...
def test_submodel_dict():
    house = pydantic_plus.obj_from_toml(HOUSE_TOML_DICTDIM_PATH, HouseDictDim, convert_strpaths=True)
...
def test_submodel_toml_badpath():
    with pytest.raises(FileNotFoundError):
        pydantic_plus.obj_from_toml(HOUSE_TOML_BAD_DIMPATH_PATH, House, convert_strpaths=True)
